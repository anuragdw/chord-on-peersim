 ...
 * 
 */
public class Observer2 implements Control {
	 ...
	 * 
	 */
	public Observer2(String prefix) {
		this.prefix = prefix;
	...
	public boolean execute() {
		...
		for (int i = 0; i < size; i++) {
			...
			//if(cp.anchor) System.out.println(" ANU "+ i );
			...
			cp.stabilizations = 0;
			cp.fails = 0;
			int maxNew = cp.getN();   //maxArray(counters, cp.index);
			if (maxNew > max)
				max = maxNew;
