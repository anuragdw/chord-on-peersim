...
public class CUPChordInitializer implements NodeInitializer {
	...
	public CUPChordInitializer(String prefix) {
	...
	public void join(Node myNode) { 
		...
		if (cpSucc.cup_stale == false) {

			if ( (idInabC(cp.chordId, rValueId, cpSucc.chordId)) || (rValueId.compareTo(cpSucc.chordId) == 0 ) ) {
				System.out.println("# node : From " + cp.successorList[0].getID() + " to " + myNode.getID());
				cpSucc.cup_stale = true;
				cp.cup_stale = false;
				cp.cup_val = cpSucc.cup_val;
				cp.cup_vet = cpSucc.cup_vet;

				//BigInteger tmpId = ((ChordProtocol) message.getFrom().getProtocol(pid)).chordId;
				CUPListObject tmpEntry = new CUPListObject( cpSucc.chordId, true, cp.successorList[0], true); 
				cp.cup_list.put( cpSucc.chordId, tmpEntry);	
		...
		try {  
			...
			CUPRequestMessage message = new CUPRequestMessage (myNode,myNode,targetId); 
