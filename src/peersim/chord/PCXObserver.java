 ...
 * 
 */
public class PCXObserver implements Control {
...
//
	 ...
	 * 
	 */
	public PCXObserver(String prefix) {
		this.prefix = prefix;
	...
	public boolean execute() {
		...
		for (int i = 0; i < size; i++) {
			...
			*/
			...
			if (cp.pcx_vet > CommonState.getTime() ) count ++;
			if (cp.pcx_hop > 0 ) {
				hnc++;
				hc += cp.pcx_hop;
				cp.pcx_hop = 0;
...
//			if ( CommonState.getTime() >= 5000 ) 
			System.out.println(CommonState.getTime()/1000.0  + " " + size + " " + count + " " + hc + " " + hnc + " " + (float)hc/hnc) ; 
